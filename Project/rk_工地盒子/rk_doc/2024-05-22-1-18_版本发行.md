...
# 停止任务
curl -X POST --data "{\"id\":\"Y2k91\"}" http://127.0.0.1:8000/api/setting/task/stop

# 删除任务
curl -X POST --data "{\"ids\":[\"A3lLD\"]}" http://127.0.0.1:8000/api/setting/task/delete

# 启动任务
curl -X POST --data "{\"id\":\"A3lLD\"}" http://127.0.0.1:8000/api/setting/task/start
