...
+ 11.17
  + 在GitLab建立分支，确保与3公斤臂测试、演示环境完全一致。 [完成]
    + vcr-dev分支，与三公斤机械臂演示环境 11月17日上午 代码完全一致
  + 在连接10公斤臂的测试机，搭建测试环境。 [完成]
  + 设备管理，机械臂点动控制，机械臂托动示教，实机联调。
  + 新增动态物体测速算法。 [最高优先级]
    + vcr-dev 作为演示环境的备份，非必要不再更改
    + master分支继续开发，增加测速接口
    + 接口协议
      + 输入参数： 
        + 超时时间，表示超过该时间未成功检测到速度，返回失败，即最大速度检测时间。单位为 秒
        + 检测区域，避免检测到不完整的目标
      + 输出数据： 
        + 检测速度，
        + 物体运动方向
      + 逻辑
        + 拍一张，取运动方向上最远的目标，当作目标第一帧
        + 再拍一张，取运动方向上最远的目标，当作目标第二帧
        + 两帧做差，求位移差和时间差，求出速度