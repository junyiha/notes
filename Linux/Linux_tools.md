...
+ tmux（terminal multiplexer）是Linux上的终端复用神器，可从一个屏幕上管理多个终端。使用该工具，用户可以连接或断开会话，而保持终端在后台运行。
...
+ `Ctrl+b "`：划分上下两个面板。
+ `Ctrl+b x`：关闭当前面板。
+ `Ctrl+b `：光标切换到其他面板。是指向要切换到的面板的方向键，比如切换到下方面板，就按方向键↓。
+ `Ctrl+b ;`：光标切换到上一个面板。
+ `Ctrl+b o`：光标切换到下一个面板。
+ `Ctrl+b {`：当前面板左移。
+ `Ctrl+b }`：当前面板右移。
...
+ `Ctrl+b Alt+o`：当前面板下移。
+ `Ctrl+b !`：将当前面板拆分为一个独立窗口。
+ `Ctrl+b z`：当前面板全屏显示，再使用一次会变回原来大小。
+ `Ctrl+b Ctrl+`：按箭头方向调整面板大小。
+ `Ctrl+b q`：显示面板编号。
